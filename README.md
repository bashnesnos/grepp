...
Once pulled you can run "$grepp_home/gradle test" to gather dependencies and see if it compiles and all is good.
...
For deeper configuration fill in *_tmplt files and remove "_tmplt" suffix in $grepp_home/dev or $grepp_home/release folders. 
Each of those will be applied for packaging via "gradle installDev" or "gradle install" correspondingly.

Try "grepp -?" for further clues
To make some configuration from scratch having just your ".poperties" file, use "grepp --parse logconfig.properties". It'll fill config.xml and give you a hint.
